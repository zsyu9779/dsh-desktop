...
- **预装常用插件**：启动时把 open-editor、diff-review、account-login、webview-compat、webview-links 装进 DSH profile，并做版本跟踪与升级；上一个版本预装的 file-changes、subagent-max 与 agent-preset-compat 会在升级时自动移除。
...
- 通过带一次性配对码的二维码完成设备配对，成功后写入 HttpOnly 设备凭据
- 重新生成配对码只会替换尚未使用的配对码；已配对设备需在设备列表中单独撤销
- 关闭手机远程后会停止局域网代理并清除待使用的配对码，已配对设备记录会保留

> 当前版本通过自签名 HTTPS 在**可信局域网**内直连，并显示证书指纹供核对；它不是公网远程访问。请勿直接暴露到互联网。
...
应用默认固定上游版本 `@deepseek-ai/dsh@0.1.5-rc.3`，避免每次启动因上游最新版变化而产生不可预测的行为。
