...
- On launch, uses `npm exec` to bootstrap `pnpm@11.7.0`, then starts `@deepseek-ai/dsh@0.1.7-rc.1 web --no-open` on a managed port (prefers `3080`, falls back to a random port if taken)
...
- Go 1.23+
- Node.js 22.19+ on the 22.x line, or Node.js 24.2+ on the 24.x line
